"""A0 source-to-mesh experiment, deliberately outside the production path.
...
from maniml.web.geometry import SURFACE_DTYPE, _jsonable, _stroke_verts
from maniml.web.triangle_geometry import TessellationError


class UnsupportedPrototype(TessellationError):
    """This A0 implementation cannot promise the requested appearance."""
...
class _MeshSource:
    ...
    def read(cls, mobject):
        ...
        return cls(points, np.asarray(ends), mobject.data["fill_rgba"],
                   np.asarray(mobject.get_unit_normal()))
    ...
    def matches(self, other):
        ...
        return all(np.array_equal(a, b) for a, b in zip(self.arrays(), other.arrays()))
...
class _MeshGeometry:
    ...
    def pixel_error(self, source, uniforms, resolution):
        ...
        residual = planar_projection_error(source.points, self.fitted_points,
                                           uniforms, resolution)
        scale = projection_scale_bound(self.fitted_points, self.basis,
                                       uniforms, resolution)
...
class TriangleMeshCache:
    """Optional A0 retained fill meshes, bounded by array bytes and entry count.

    Reuse requires exact source/contour/normal content and the same
    ...
    versions. This bounds plane fitting plus flattening, not float32 arithmetic,
    antialiasing, total CPU time, or tessellator scratch allocations.
...
def projection_scale_bound(points, basis, uniforms, resolution):
...
def mesh_mobject(mobject, tessellator, uniforms, resolution, pixel_tolerance=0.25):
    ...
    geometry = _generate_mesh(_MeshSource.read(mobject), tessellator, uniforms,
...
def _generate_mesh(source, tessellator, uniforms, resolution, pixel_tolerance,
    ...
    tolerance = plane_tolerance(fitted_points, basis, uniforms, resolution,
    ...
    mesh = tessellator.tessellate(
        ...
        tolerance=float(tolerance32), fill_rule="nonzero")
    ...
    return _MeshGeometry(vertices, mesh.indices, fitted_points, basis, float(mesh.tolerance))


def prepare_triangle_frame(scene, tessellator, *, pixel_tolerance=0.25,
                           diagnostic=False, mesh_cache=None, fill_builder=None):
    """Prepare complete painter-ordered draws; no production shader changes.

    A0 supports planar vector fills, existing strokes, untextured surfaces and
    dot clouds. Per-point fill paint, fill borders, and fill lighting remain gaps.
    Images/textured surfaces wait for the shared material integration milestone.
    ...
    A separate A0 candidate can supply fill_builder(mobject, uniforms,
    ...
    for group in scene.render_groups:
        for sm in sorted(group.family_members_with_points(), key=lambda obj: obj.z_index):
            if isinstance(sm, CameraFrame):
                continue
            uniforms = {k: _jsonable(v) for k, v in
                        {**camera.uniforms, **sm.uniforms}.items()}
            depth_suffix = "_depth" if sm.depth_test else ""
            if isinstance(sm, TexturedSurface):
                raise UnsupportedPrototype("textured surfaces await material integration")
            if isinstance(sm, (DotCloud, Surface)):
                data = np.ascontiguousarray(sm.get_shader_data()).copy()
                pipeline = "dot" if isinstance(sm, DotCloud) else "surface"
            ...
            if np.any(sm.data["fill_rgba"][:, 3]):
                ...
                if np.any(sm.data["fill_border_width"]):
                    limitation("fill-border coverage is not represented in the A0 mesh")
                ...
                if fill_builder is not None:
                    ...
                    if len(indices):
                        fill = TriangleDraw("surface" + depth_suffix, vertices, uniforms,
                                            indices, len(indices))
            stroke = None
            if np.any(data["stroke_width"]) and np.any(data["stroke_rgba"][:, 3]):
