...
2. **Source map** (`scene/source_map.py`): parses the scene file with AST and splits `construct()` into **animation units** — runs of consecutive top-level statements ending with a *stop statement*. Which call is a stop depends on the file (`pause_anchored`): a file that calls `self.pause()` / `self.next_section()` anywhere is **pause-anchored** — pauses are the stops and any number of plays between them runs through as one stretch; a file with no pauses keeps the legacy anchoring where every `.play(...)` is a stop, so unmodified CE files stay steppable. A stop inside a for-loop/if keeps its whole enclosing statement. Trailing statements after the last stop (e.g. `self.wait()`) form a tail unit with `has_stop=False`. Pause detection requires the `self.` spelling, so a scene about a media player calling `player.pause()` cannot flip the mode. Unit-tested in `tests/test_source_map.py`.
...
3. **Scene / checkpoints** (`scene/scene.py` + `scene/checkpoints.py`): `Scene` is composed from mixins — `CheckpointMixin` (`checkpoints.py`: checkpoint save/restore, unit re-execution, watcher replay, `deepcopy_namespace`), `InteractionMixin` (`interaction.py`: key/mouse handlers, reverse morph, click-to-inspect/drag), `PresentationMixin` (`presentation.py`: present mode, timeline, render mode) — with `scene.py` keeping the core lifecycle, `SceneState`, and `ThreeDScene`.
   ...
   - Each checkpoint anchor — `play()` in a plain file, only `pause()` in a pause-anchored one — saves `{index, line_number, unit_index, run_time, name, state, namespace}` — namespace and scene state are deep-copied *together* (`deepcopy_namespace`) so variable↔mobject references survive. In pause-anchored mode a play just extends the stretch run-time accumulator, whose total the pause's checkpoint records so LEFT-stepping back takes the stretch's real span; `pause()` works from helpers and loop bodies since it saves at call time. `pause(loop=True)` flags its checkpoint as a looping hold: the interact loop (`_maybe_replay_loop_pause`, live viewer with a client only) replays the unit that led in, over and over, until any arrow parks the scene elsewhere — render/export never enter `interact()`, so it is an ordinary pausepoint there. Adding the first pause (or removing the last) on a watcher save rebuilds the scene — every unit index belongs to the other regime.
