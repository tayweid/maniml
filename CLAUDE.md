...
- `static/` — `viewer.html` and `app.html` (each one file; shared tokens *and the pod run* in `shell.css` — the chrome is Plass's toolbar with its metrics, not an imitation: 60px bar, 42px pods pressed into a run with stadium ends, contents asleep until the pointer nears, flyouts as pills laid over their trigger), `gl.js`/`glsl/` (WebGL2), `webgpu.js`/`wgsl/` (WebGPU), `player.*` (the baked player, deliberately standalone — no `shell.css`), `manifest.webmanifest` + `sw.js` (the installable local app).
...
**Wire protocol.** Server→client: binary frames — 1 header byte, 0x01 JPEG / 0x02 PNG (image GL-bottom-up, so the client flips via canvas transform), 0x03 geometry — plus state JSON `{current, count, lines, future}` and move JSON `{from, to, back}`. Client→server: key/pointer/chip JSON, pointer coords normalized to the frame [0,1] y-up, so no window-size bookkeeping.
