...
| [phase_b3_plan.md](phase_b3_plan.md) | B3: animations as GPU programs over control-point rows (blend, affine, paint, partial), shadow mode then the flip with materialize-on-read, the clock deferred with the frame loop. Proposed; B3a (the blend) is first. |
