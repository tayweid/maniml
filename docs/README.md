...
| [phase_b3_plan.md](phase_b3_plan.md) | B3: animations as GPU programs over control-point rows (blend, affine, paint, partial), shadow mode then the flip with materialize-on-read, the clock deferred with the frame loop. B3a (the blend) and B3b (affine, paint, partial) built 2026-09-11 in both drivers behind `MANIML_PROGRAMS`, with measured results and the two fill fixes the gate found; B3c next. |
