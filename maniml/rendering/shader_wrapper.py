"""Native OpenGL wrappers, owned by the explicitly selected GL camera."""
...
from maniml.rendering.gl_shaders import get_shader_code_from_file
from maniml.rendering.gl_shaders import get_shader_program
from maniml.rendering.gl_shaders import image_path_to_texture
from maniml.rendering.gl_shaders import set_program_uniform
...
if TYPE_CHECKING:
    ...
    from maniml.typing import UniformDict
...
class ShaderWrapper(object):
    def __init__(
        ...
        render_primitive: int = 5,  # GL_TRIANGLE_STRIP
        code_replacements: dict[str, str] = dict(),
    ):
        # Public class imports remain context-free; load GL only for a real wrapper.
        global gl, moderngl
...
class VShaderWrapper(ShaderWrapper):
    def __init__(
        ...
        texture_paths: Optional[dict[str, str]] = None,  # A dictionary mapping names to filepaths for textures.
        depth_test: bool = False,
        render_primitive: int = 4,  # GL_TRIANGLES
        code_replacements: dict[str, str] = dict(),
    ...
    def get_fill_canvas(ctx: moderngl.Context) -> Tuple[Framebuffer, VertexArray, Framebuffer]:
        ...
        resources = context_resources(ctx)
        if "fill_canvas" in resources:
            return resources["fill_canvas"][0]
        ...
        result = (fill_texture_fbo, fill_texture_vao, depth_texture_fbo)
        resources["fill_canvas"] = (result, (fill_texture, depth_texture, fill_program, simple_vbo))
        return result
