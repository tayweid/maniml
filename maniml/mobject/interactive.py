...
from maniml.mobject.geometry import RoundedRectangle
from maniml.mobject.geometry import Square
from maniml.mobject.types.vmobject_3d import Text
...
from maniml.utils.space_ops import get_closest_point_on_line
...
class Textbox(ControlMobject):
    ...
    def on_key_press(self, mob: Mobject, event_data: dict[str, int]) -> bool | None:
        ...
        if mob.isActive:
            ...
            new_value = old_value
            if char.isalnum():
                if (modifiers & WindowKeys.MOD_SHIFT) or (modifiers & WindowKeys.MOD_CAPSLOCK):
                    new_value = old_value + char.upper()
                else:
                    new_value = old_value + char.lower()
            elif symbol in [WindowKeys.SPACE]:
                new_value = old_value + char
            elif symbol == WindowKeys.TAB:
                new_value = old_value + '\t'
            elif symbol == WindowKeys.BACKSPACE:
