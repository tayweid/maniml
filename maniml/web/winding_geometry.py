"""Original winding serializer retained for viewer dogfood comparisons.
...
def _triangulated_fill_data(sm):
    ...
    triangulation = _legacy_triangulation(sm)
...
# Both selectable renderers use the same bounded, stat-validated texture reader.
# Serialization pins current bytes separately, so cache eviction while collecting
# another image cannot remove a payload this frame still needs.
from maniml.web.geometry import _texture_refs, _texture_file
...
def _collect_records(scene, unsupported, texture_payloads=None):
            ...
            if isinstance(sm, (DotCloud, ImageMobject, Surface)):
                if isinstance(sm, DotCloud):
                    ...
                    record = plain_record(sm, "image", 24,
                                          textures=_texture_refs(sm, texture_payloads))
...
def serialize_scene(scene: Scene, cache: GeometryCache | None = None, *,
    ...
    frame_textures: dict[str, bytes] = {}
    current_hashes: set[str] = set()
    texture_hashes: set[str] = set()

    with performance.stage("geometry.collect_shader_data"):
        records = _collect_records(scene, unsupported, frame_textures)
        ...
        for record, fill_rect in zip(records, fill_rects):
            ...
            if record["textures"]:
                ...
                for tex_hash in record["textures"].values():
                    texture_hashes.add(tex_hash)
                    if cache is None or f"tex:{tex_hash}" not in cache.sent:
                        needed_textures[tex_hash] = frame_textures[tex_hash]
